---
title: "research-init"
description: "Use when onboarding a repository onto the honest-scholar research workflow — either scaffolding a fresh repo (init) or backfilling an existing one that already has papers, datasets, benchmarks, and…"
---
...
[§5](/design/00-meta-spec) and the content layout in
[lifecycle §7](/design/01-lifecycle)). `adopt` is `init` **plus** an
...
([ADR-0018](/decisions/0018-git-native-source-of-truth)). The plugin
...
[`resources/templates/`](https://github.com/davorrunje/honest-scholar/tree/main/resources/templates); every hypothesis/paper/thesis
...
- **Delegates per-item registration to the capability skills' verbs** — it does
  ...
  [`literature`](/skills/literature) (`scout` / `position`),
  [`dataset`](/skills/dataset) (`init/register/fetch/verify/mirror/audit/export`),
  and the pipeline skills
  ([`hypothesis-exploration`](/skills/hypothesis-exploration),
  [`hypothesis-testing`](/skills/hypothesis-testing),
  [`paper-exploration`](/skills/paper-exploration),
  [`paper-synthesis`](/skills/paper-synthesis)). In `adopt`, the mapping
...
- **Human confirms every material classification.** In `adopt`, licenses/tiers
  ...
  propose, never presume ([ADR-0017](/decisions/0017-research-init-one-skill),
...
- **Git-native only.** Do not introduce an external tracker as the source of
  ...
  ([ADR-0018](/decisions/0018-git-native-source-of-truth)).
...
- **Do not commit** as part of this skill — leave the scaffolding staged for the
  author to review and commit ([ADR-0001](/decisions/0001-separate-plugin-repo)
