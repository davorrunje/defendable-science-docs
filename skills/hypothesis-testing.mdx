...
description: "Use when driving one promoted hypothesis from a claim to a signed verdict — the resolve skill at the hypothesis level. Runs the staged pipeline hypothesis → strategy → design/plan → findings, does the…"
---
...
[`hypothesis-exploration`](/skills/hypothesis-exploration) (explore proposes,
...
[`progress`](/skills/progress). Templates live in
[`../../resources/templates/`](https://github.com/davorrunje/honest-scholar/tree/main/resources/templates).
...
2. **`strategy.md`** — *the science, before any engineering.* This is the heart of
   ...
   - The **rigor-kit choices** (below): confirmatory-vs-exploratory tag,
     ...
     the claim is a null.
   - Calls [`literature position --level hypothesis`](/skills/literature) — an
   ...
   - Declares the **datasets** it will use; materialize/verify them via the
     [`dataset`](/skills/dataset) skill so their fingerprints are pinned.
...
([`../../resources/contracts/experiment-backend.md`](https://github.com/davorrunje/honest-scholar/blob/main/resources/contracts/experiment-backend.md)),
...
`findings.md`. Every element is also a [`defend`](/skills/defend) `methodology`
target — you must be able to explain *why*, not merely perform the ritual (checklists
live in [`../../resources/rigor/`](https://github.com/davorrunje/honest-scholar/tree/main/resources/rigor); the *why* is grounded in
[`../../resources/references/understanding-and-defense.md`](/references/understanding-and-defense)).
...
- **Per-dataset datasheets** — Gebru-style, produced via the
  [`dataset`](/skills/dataset) skill for every dataset the strategy declares.
- **Red-team pass** — adversarial self-review of the strategy and the findings
  before the verdict is signed.

## Composition

- [`hypothesis-exploration`](/skills/hypothesis-exploration) — upstream; a
  promoted backlog item seeds `hypothesis.md`.
- [`literature`](/skills/literature) — `position --level hypothesis` in the
  strategy stage (adversarial precedent review).
- [`dataset`](/skills/dataset) — materialize, verify, and datasheet every
  declared dataset.
...
- [`defend`](/skills/defend) — strategy target (during strategy) and guardrail
  (before the findings sign-off).
- [`progress`](/skills/progress) — reads this hypothesis's status frontmatter;
...
discovery + provenance (see [`../../resources/commit-attribution.md`](https://github.com/davorrunje/honest-scholar/blob/main/resources/commit-attribution.md)):
