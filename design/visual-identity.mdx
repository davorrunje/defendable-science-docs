...
description: "Honest Scholar's identity is built to read as rigorous, not decorative: a deep indigo grounds the system in credibility, a single coral accent carries every…"
---
...
[`assets/visual-identity.md`](https://github.com/davorrunje/honest-scholar/blob/main/assets/visual-identity.md); keep the two in
sync (palette and asset roles here must match it).

## Logo

![Wordmark lockup on a light surface](/wordmark-lockup-light.svg)

The icon mark ([`assets/icon-mark.svg`](/icon-mark.svg)) is a coral
...
- [`wordmark-lockup-light.svg`](/wordmark-lockup-light.svg) — indigo
  wordmark, for **light** surfaces (docs in light mode, README body).
- [`wordmark-lockup-dark.svg`](/wordmark-lockup-dark.svg) — white
...
- **README header** — the banner ([`assets/wordmark-banner.svg`](https://github.com/davorrunje/honest-scholar/blob/main/assets/wordmark-banner.svg)),
  centred at ~640px.
- **Docs site** — `docs.json` uses the indigo primary, the light-surface lockup
  ...
  the favicon. The build tool ([`tools/build_docs_site.py`](https://github.com/davorrunje/honest-scholar/blob/main/tools/build_docs_site.py))
...
- **Badges** — flat-square style, `labelColor=241852` (indigo). Status/metric
  ...
  (coral). See the shields.io URLs in the root [`README.md`](/index).
- **Social preview** — [`assets/social-preview.svg`](/social-preview.svg)
