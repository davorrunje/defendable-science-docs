---
title: "honest-scholar — User Guide"
description: "A hands-on guide for a researcher meeting honest-scholar for the first time. It walks you from install to a signed finding to a submitted paper, using one…"
---
...
For the *why* behind the design, read [`README.md`](/index) and the specs
under [`docs/design/`](/design/00-meta-spec); this guide is the *how*.
...
2. **Understanding — the `defend` skill verifies and teaches.** Before any material decision,
   the [`defend`](/skills/defend) skill probes whether you can actually
...
Onboard your repo once with [`research-init`](/skills/research-init).
...
[`hypothesis-exploration`](/skills/hypothesis-exploration) runs an
...
[`hypothesis-testing`](/skills/hypothesis-testing), the resolve skill.
...
- [`literature`](/skills/literature) has two modes over one
...
- [`dataset`](/skills/dataset) manages the data your strategy declares:
...
- [`paper-exploration`](/skills/paper-exploration) proposes *candidate
...
- [`paper-synthesis`](/skills/paper-synthesis) develops the committed
...
[`paper-synthesis`](/skills/paper-synthesis) **proactively proposes** the
...
level entirely. [`thesis`](/skills/thesis) is one skill with two modes:
...
- [`progress`](/skills/progress) is **read-only reporting**. `status
...
- [`defend`](/skills/defend) is the Socratic tutor-examiner. It is
...
|---|---|---|---|
| `S2_API_KEY` | Semantic Scholar | Raises the rate limit far above the shared keyless pool (which throttles hard). | [https://www.semanticscholar.org/product/api#api-key](https://www.semanticscholar.org/product/api#api-key) |
| `OPENALEX_MAILTO` | OpenAlex | Joins the "polite pool" (a contact email) for faster, more reliable responses. | [https://docs.openalex.org/how-to-use-the-api/rate-limits-and-authentication](https://docs.openalex.org/how-to-use-the-api/rate-limits-and-authentication) |
...
- **Where the reasoning lives.** Specs (the *what*) in [`docs/design/`](/design/00-meta-spec);
  decision log with rejected alternatives (the *why*) in
  [`decisions/`](/decisions/index); verified source digests (the *evidence*) in
  [`resources/references/`](https://github.com/davorrunje/honest-scholar/tree/main/resources/references); fillable staged-doc
  skeletons in [`resources/templates/`](https://github.com/davorrunje/honest-scholar/tree/main/resources/templates).
